...
class KitaLdrRepository(private val context: Context) {
    ...
    fun uploadProfilePhoto(sourceUri: Uri, onResult: (Result<String>) -> Unit) {
        ...
        Thread {
            try {
                val compressedFile = File(requireNotNull(compressProfileImage(sourceUri).path))
                val path = "profile/$uid/${System.currentTimeMillis()}.jpg"
                val bucket = SupabaseStorage.client.storage.from("avatars")

                bucket.upload(path, compressedFile) {
                    upsert = false
                }
